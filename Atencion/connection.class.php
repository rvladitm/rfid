...
class Common{
	...
	public function validateLogin(){
		if(isset($_POST)){
			...
			if(!isset($username) || !isset($password)){
				$this->setErrorMsg("Ususario o contraseña están vacíos");
			...
			if($chkUser[0]['userStatus']>0){
				...
				$this->setErrorMsg("Usuario no existe, intente nuevamente");
				return ;
			}
		}
	}
	public function getAttendanceData($month,$year){
		if(!is_numeric($month) || $month>12 || $month<1){
			$this->setErrorMsg("Seleccione un mes válido");
			header("location:atencion.php");
			exit;
		}
		$retData = array();
	...
	public function getRFIDAttendanceApi(){
		...
		if(count($data)>0 && $data[0]['tot']>0){
			...
			if($flag){
				echo "Atencion marcada satisfactoriamente";
				die;
			}else{
	...
	public function addUser(){
		...
		if(empty($name) || empty($rfid_uid)){
			echo "Nombre no puede quedar vacío";
			exit;
		}
		$sql = "insert into tbl_users set name = '$name',rfid_uid = '$rfid_uid'";
		if($this->conn->query($sql)){
			echo "Añadido Satisfactoriamente";
			exit;
		}else{
			echo "Error";
			exit;
